...
const PostManager = ({ action, posts, handleCloseModal, handleChangeStatusPost, handleShowModal }) => {
            ...
            {[...paginate.data].map(post => (
              <tr key={post.id} className='post-item__row'>
                ...
                <td className='pir__data--action'>
                  <span className='pir__data__act--approve'
                    onClick={() => handleShowModal(ActionType.APPROVE, post)}
                  >Approve</span>|
                  <span className='pir__data__act--reject'
