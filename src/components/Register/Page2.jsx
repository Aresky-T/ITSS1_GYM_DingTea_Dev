    ...
    <div className="page2-main">
      <div className="flex-container register__upload-file">
        <div className="upload-gym-images">
          <div className="rgi__title">画像をアップロード</div>
          <div className="register__main" onClick={onClickChooseImage}>
            {[...selectedFiles.imageFiles].length === 0 ? (
              <>
                <div className="rgm__choose-file">ファイルを選ぶ</div>
                <input
                  type="file"
                  name="imageFiles"
                  multiple
        ...
        <div className="upload-gym-logo">
          <div className="rgi__title">ロゴをアップロード</div>
          <div className="register__main" onClick={onClickChooseLogo}>
            {[...selectedFiles.logoFile].length === 0 ? (
              <>
                <div className="rgm__choose-file">ファイルを選ぶ</div>
                <input
              ...
              {options.map((opt) => (
                <div className="rgm_options__item" key={opt.title}>
                  ...
                  <div className="rgm_opt-details">
                    <h4 className="rgm_opt__title">{opt?.title}</h4>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="register-item register__description">
        <div className="rgi__title">説明</div>
        <div className="register__main">
          <textarea
            cols="30"
            rows="10"
      ...
      {isShowModal && (
        <AddOptionModal page2={page2} setIsShowModal={setIsShowModal} />
      )}
    </div>
  );
};
